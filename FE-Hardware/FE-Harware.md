...
CPU执行一条指令的完整流程可以分解为以下四个基本步骤：

1. **指令取出 (命令の取り出し)**
    控制装置根据程序计数器中存储的地址，从主存中获取指令，并将其存入指令寄存器。完成后，程序计数器的值会自动增加，指向下一条指令。
2. **指令解码 (命令の解読（かいどく）)**
    指令寄存器中的指令被送到解码器 (デコーダ)。解码器会解析这条指令，确定它到底是要做什么操作（例如，加法、数据移动等），并通知运算装置准备执行。
3. **操作数读取 (オペランド読み出し)**
    如果指令需要操作数据（例如，加法需要两个数），CPU会根据指令中的操作数部分 (オペランド部) 来获取这些数据。数据的来源可能是寄存器，也可能是主存。
4. **指令执行 (命令実行)**
    运算装置 (ALU) 根据解码后的指令执行具体的计算或操作。如果产生了结果，这个结果会被存回通用寄存器或主存中。

完成这四步后，CPU会回到第一步，开始执行下一条指令，周而复始。
![CPU指令执行的四个步骤](Image/workflow.png)

## 2.3 地址指定方式 (アドレス指定方式)

“地址指定方式”指的是指令的操作数部分 (オペランド部) 用来指定操作数据位置的不同方法。选择合适的方式可以提高程序效率和灵活性。

| 方式名称 | 日文名称 | 原理说明 |
| :--- | :--- | :--- |
| 立即寻址 | 即時アドレス指定方式 | 操作数部分直接包含数据本身。 |
| 直接寻址 | 直接アドレス指定方式 | 操作数部分是数据所在的内存地址。 |
| 间接寻址 | 間接アドレス指定方式 | 操作数部分是存放数据地址的内存地址（地址的地址）。 |
| 变址寻址 | インデックスアドレス指定方式 | 操作数地址与变址寄存器的值相加，得到有效地址。 |
| 基址寻址 | ベースアドレス指定方式 | 操作数地址与基址寄存器的值相加，得到有效地址。 |
| 相对寻址 | 相対アドレス指定方式 | 操作数地址与程序计数器的值相加，得到有效地址。 |
...
主存储器（通常指内存）可分为两大类：RAM (ラム) 和 ROM (ロム)。

* **RAM (随机存取存储器)**: 数据可以被自由读写，但断电后数据会丢失，是一种易失性存储器 (挥发性)。
* **ROM (只读存储器)**: 通常只能读取，数据在断电后不会丢失，是一种非易失性存储器 (不挥发性)。
...
* **显示器关键概念**
  ...
  * **VRAM**: Video RAM，专门用来存储即将显示在屏幕上的图像内容的内存。
* **打印机关键性能指标**
